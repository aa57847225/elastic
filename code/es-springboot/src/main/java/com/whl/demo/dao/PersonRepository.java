...
public class PersonRepository {
    ...
    public boolean buildPersonIndex() {
        try {
            ...
            PutMappingRequest putmap = Requests.putMappingRequest("person").type("xwc").source(mapping);
            //创建索引
            client.admin().indices().prepareCreate("person").setSettings(mapping2).execute().actionGet();
            //为索引添加映射
    ...
    public String addPersonIndexDataBatch(List<Person> personList) {
        ...
        try {
            ...
            for (Person person : personList) {
                //插入
                bulkRequest.add(this.client.prepareIndex("person", "xwc", person.getId())
                        .setSource(XContentFactory.jsonBuilder()
                                .startObject()
